...
def parse_config(spec: str) -> tuple[int, int, int]:
    ...
    if len(parts) != 3:
        raise argparse.ArgumentTypeError(
            f"Config must be envs:n_steps:batch_size, got '{spec}'"
        )
...
def main() -> None:
    ...
    print("=" * 85)
    print(f"CPU: {psutil.cpu_count(logical=False)} physical / "
    ...
    for num_envs, n_steps, batch_size, use_subproc in configs:
        ...
        r = measure_training(
            num_envs, n_steps, batch_size, args.timesteps,
            use_subproc, args.seed,
        )
    ...
    print()
    print("=" * 85)
    print(f"{'Envs':>4} {'nSteps':>6} {'Batch':>5} {'Backend':>7} {'Buffer':>6} "
    ...
    for r in results:
        ...
        print(f"{r['num_envs']:>4} {r['n_steps']:>6} {r['batch_size']:>5} "
    ...
    print()
    print(f"BEST: {best['num_envs']}e/{best['n_steps']}s/{best['batch_size']}b/"
